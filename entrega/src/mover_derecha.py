...
def dodge_and_follow():
    rospy.init_node ('move_aprendizaje')
    ...
    while not rospy.is_shutdown():
        ...
        """if range_right>range_left and range_right>range_ahead:
            print ("distancia derecha ",range_right)
        elif range_left>range_right and range_left>range_ahead:
            print ("distancia izquierda ",range_left)
        else:
            print ("distancia delante ",range_ahead)"""
        ...
        v_giro = abs( (umbral_derecho - range_60) + 0.25 ) *0.1
        ...
        if range_ahead > 0.5 and range_right < 1.5: #el robot se debe mover mientras no detecte un objeto al frente y al encontrarse cerca de una pared.
            ...
            move.linear.x = 0.5
            move.angular.z = -1.0 #-v_giro (necesito ajustarla mejor de manera que se comporte mejor en ciertas partes)
        ...
        if range_60 < 1.6:   #revisar valores de distancia
            move.linear.x = 0.3
            move.angular.z = 0.8
